...
<script>
    ...
    import {getChinaJson, getProvinceJSON,getResultJson} from "../../api/get-json";
    ...
    export default {
        ...
        methods: {
            ...
            async requestGetChinaJson() {
                ...
                let resultJson = await getResultJson();
                // let hStep = 300 / (resultJson.length - 1);
                let obj1 = eval(resultJson);// 解析json数据
                let busLines=[];
                for(let i=0;i<obj1.length;i++) {
                    let points = [];
                    for (let j = 0; j < obj1[i].Mnst.length; j++) {
                        // 这里有一个坑我必须说明一下 我的数据result8.json里经纬度反了，所以这里取值的时候先取的value[1],再取的value[0]
                        points.push([obj1[i].Mnst[j].value[1], obj1[i].Mnst[j].value[0]]);
                    }
                    // 这里在组装连线的数据，下面将用到
                    busLines.push({
                        coords: points,// 坐标这个数据是必须给的
                        name: obj1[i].name,// 类似name还可以根据自己的需求给更多的数据
                ...
                //绘制地图
                this.renderMap('china', arr,busLines);
            },
            // 加载省级地图
